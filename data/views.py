...
def profile(request, id=None):
    ...
    return render(request, "profile.html", { "victim": victim } )


def village(request, id=None):
    victims = Data.objects.filter(village_id=id).order_by('victim_name')
    if id is not None and id is None:
        return messages.warning(request,"Village %s was not found"%id)
    return render(request, "village.html", { "victims": victims } )
